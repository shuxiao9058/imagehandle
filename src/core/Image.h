...
#ifndef _MATLAB
	virtual bool loadgray(const char* filename);
...
template <class T>
bool Image<T>::loadgray(const char* filename)
{
	if(!imread(filename))
		return false;
	if (1 == nChannels)
	...
	T *pDt = new T[imWidth * imHeight * nChannels];
	memcpy(pDt, pData, imWidth * imHeight * nChannels * sizeof(T));
	delete []pData;
	nChannels = 1;
	
	pData = new T[imWidth * imHeight];

	T *p = pData;
	for (int i = 0; i < imWidth * imHeight; ++i)
	{
		*p++ = (pDt[i * 3] + pDt[i * 3 + 1] + pDt[i * 3 + 2])/3;
	}
	delete[] pDt;
...
{
	if (maxlevel < 1) {
		fprintf(stderr, "level must great than 1 or equal to 1.");
		return;
	}
